...
- Taiwan Creative Content Agency fallback: this source often returns 403 or times out. When the primary list URL fails, retry once with browser-like request headers, a warm-up request to `https://www.taicca.tw/`, and a 60-90 second timeout. If it still fails, try `https://www.taicca.tw/public_information/purchase_placard/list`, then official detail probing around the newest known numeric detail ids under `https://www.taicca.tw/public_information/purchase_placard/detail/<id>` and `https://taicca.tw/public_information/purchase_placard/detail/<id>?type_id=1`. If the homepage warm-up itself returns 403, skip repeated direct detail probing for that run and move directly to the web-search fallback. Include only official detail pages whose `公告類別` is `招標公告` and whose `公告日期` is inside the report date range. If both list and detail probing fail, use web search queries limited to official TAICCA purchase placard URLs, such as `site:taicca.tw/public_information/purchase_placard/detail 文策院 招標公告 <year>` and date-specific queries, then mark the item as `搜尋引擎快照，需人工確認`. Do not mark the TAICCA source as fully successful unless at least one official list or official detail page was read directly.
